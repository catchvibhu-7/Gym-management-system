...
router.post('/workout-plans', (req, res) => {
  ...
  const insertEx = db.prepare(
    `INSERT INTO workout_plan_exercises (plan_id, sort_order, name, sets, reps, weight_note, rest_seconds, notes)
     VALUES (?,?,?,?,?,?,?,?)`
  );
  exercises.forEach((e, i) => insertEx.run(planId, i, e.name, e.sets || null, e.reps || null, e.weightNote || null, e.restSeconds || null, e.notes || null));
...
router.put('/workout-plans/:id', (req, res) => {
  ...
  exercises.forEach((e, i) => insertEx.run(plan.id, i, e.name, e.sets || null, e.reps || null, e.weightNote || null, e.restSeconds || null, e.notes || null));
